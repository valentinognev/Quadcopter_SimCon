...
def makeFigures(params, time, pos_all, vel_all, quat_all, omega_all, euler_all, commands, wMotor_all, thrust, torque, sDes_traj, sDes_calc, ulgData=None):
    ...
    for qi in range(numOfQuads):
        ...
        ydot = vel_all[:,qi,1]
        zdot = vel_all[:,qi,2]
        p    = omega_all[:,qi,0]*rad2deg
        q    = omega_all[:,qi,1]*rad2deg
        r    = omega_all[:,qi,2]*rad2deg

        wM1  = wMotor_all[:,0,qi]*rads2rpm
        wM2  = wMotor_all[:,1,qi]*rads2rpm
        wM3  = wMotor_all[:,2,qi]*rads2rpm
        wM4  = wMotor_all[:,3,qi]*rads2rpm

        phi   = euler_all[:,qi,0]*rad2deg
        theta = euler_all[:,qi,1]*rad2deg
        psi   = euler_all[:,qi,2]*rad2deg

        x_sp  = sDes_calc[:,0,qi]
        y_sp  = sDes_calc[:,1,qi]
        z_sp  = sDes_calc[:,2,qi]
        Vx_sp = sDes_calc[:,3,qi]
        Vy_sp = sDes_calc[:,4,qi]
        Vz_sp = sDes_calc[:,5,qi]
        x_thr_sp = sDes_calc[:,6,qi]
        y_thr_sp = sDes_calc[:,7,qi]
        z_thr_sp = sDes_calc[:,8,qi]
        q0Des = sDes_calc[:,9,qi]
        q1Des = sDes_calc[:,10,qi]
        q2Des = sDes_calc[:,11,qi]
        q3Des = sDes_calc[:,12,qi]    
        pDes  = sDes_calc[:,13,qi]*rad2deg
        qDes  = sDes_calc[:,14,qi]*rad2deg
        rDes  = sDes_calc[:,15,qi]*rad2deg

        x_tr  = sDes_traj[:,0,qi]
        y_tr  = sDes_traj[:,1,qi]
        z_tr  = sDes_traj[:,2,qi]
        Vx_tr = sDes_traj[:,3,qi]
        Vy_tr = sDes_traj[:,4,qi]
        Vz_tr = sDes_traj[:,5,qi]
        Ax_tr = sDes_traj[:,6,qi]
        Ay_tr = sDes_traj[:,7,qi]
        Az_tr = sDes_traj[:,8,qi]
        yaw_tr = sDes_traj[:,14,qi]*rad2deg
        
        # Extract Euler angle setpoints from trajectory (columns 12:15 are desEul)
        phi_sp_traj = sDes_traj[:,12,qi]*rad2deg
        theta_sp_traj = sDes_traj[:,13,qi]*rad2deg
        psi_sp_traj = sDes_traj[:,14,qi]*rad2deg
        
        # Extract angular rate setpoints from trajectory (columns 15:18 are desPQR)
        p_sp_traj = sDes_traj[:,15,qi]*rad2deg
        q_sp_traj = sDes_traj[:,16,qi]*rad2deg
        r_sp_traj = sDes_traj[:,17,qi]*rad2deg

        uM1 = commands[:,0,qi]*rads2rpm
        uM2 = commands[:,1,qi]*rads2rpm
        uM3 = commands[:,2,qi]*rads2rpm
        uM4 = commands[:,3,qi]*rads2rpm
        ...
        for ii in range(q0Des.shape[0]):
            YPR = utils.quatToYPR_ZYX(sDes_calc[ii,9:13,qi])
