...
def sameAxisAnimation(t_all, waypoints, pos_all, quat_all, sDes_tr_all, Ts, params, xyzType, yawType, ifsave):
    ...
    for qi in range(numOfQuads):
        ...
        def updateLines(i):
            numOfQuads = pos_all.shape[1]
            for qii in range(numOfQuads):
                time = t_all[i*numFrames]
                pos = pos_all[i*numFrames,qii]
                x = pos[0]
                y = pos[1]
                z = pos[2]

                x_from0 = np.asarray(pos_all[0:i*numFrames,qii,0]).flatten()
                y_from0 = np.asarray(pos_all[0:i*numFrames,qii,1]).flatten()
                z_from0 = np.asarray(pos_all[0:i*numFrames,qii,2]).flatten()
            
                dxm = params["dxm"]
                dym = params["dym"]
                dzm = params["dzm"]
                
                quat = quat_all[i*numFrames,qii]
